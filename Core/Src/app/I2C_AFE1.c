...
UINT8 IIC_Wait_Ack(void)
{
	UINT8 ucErrTime=0;
	SDA_IN();      //SDA设置为输入  
	IIC_SDA=1;__delay_us(DELAY_US_IIC_AFE1);
	IIC_SCL=1;__delay_us(DELAY_US_IIC_AFE1);
	while(READ_SDA)
	{
		ucErrTime++;
		if(ucErrTime>250)
...
int I2CSendBytes(unsigned char I2CSlaveAddress, unsigned char *DataBuffer, unsigned int ByteCount, unsigned int *SentByte) {
	...
	Feed_IWatchDog;
	while(I2C_GetFlagStatus(I2C_AFE, I2C_FLAG_BUSY) != RESET) {	//等待总线不忙
		if((Timeout--) == 0) {
			*SentByte = NumberOfBytesSent;
	...
	while(ByteCount--)  {
		Timeout = LONG_TIMEOUT;
		while(I2C_GetFlagStatus(I2C_AFE, I2C_ISR_TXIS) == RESET) {		//出现在发送中断，则发
			if((Timeout--) == 0) {									//等待可写数据进写缓冲寄存器
				*SentByte = NumberOfBytesSent;
...
int I2CReadBytes(unsigned char I2CSlaveAddress, unsigned char *DataBuffer, unsigned int ExpectedByteNumber, unsigned int *NumberOfReceivedBytes)
{
    ...
    *NumberOfReceivedBytesPointer = 0;					//目前没用到
	...
	Feed_IWatchDog;
	while(I2C_GetFlagStatus(I2C_AFE, I2C_FLAG_BUSY) != RESET) {	//等待总线不忙
		if((Timeout--) == 0) {									//因为PIC代码是先写再读，写函数有stop标志把TC标志位去掉
			return TIMEOUT_UserCallback();
		}
	}
	...
	while(ExpectedByteNumber) {	
		Timeout = LONG_TIMEOUT;
		while(I2C_GetFlagStatus(I2C_AFE, I2C_ISR_RXNE) == RESET) {		//收到非空，则读
			if((Timeout--) == 0) {
				return TIMEOUT_UserCallback();
			}
		}
...
#if 1
	if((bqMaximoProtectionConfig[0] != Registers_AFE1.SysCtrl1.SysCtrl1Byte)
		|| (bqMaximoProtectionConfig[1] != Registers_AFE1.SysCtrl2.SysCtrl2Byte)
		|| (bqMaximoProtectionConfig[2] != Registers_AFE1.Protect1.Protect1Byte)
		|| (bqMaximoProtectionConfig[3] != Registers_AFE1.Protect2.Protect2Byte)
		|| (bqMaximoProtectionConfig[4] != Registers_AFE1.Protect3.Protect3Byte)
		|| (bqMaximoProtectionConfig[5] != Registers_AFE1.OVTrip)
		|| (bqMaximoProtectionConfig[6] != Registers_AFE1.UVTrip)
		|| (bqMaximoProtectionConfig[7] != Registers_AFE1.CCCfg))
	{
		result = 1;
	}
#endif
